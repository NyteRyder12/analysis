...
R__LOAD_LIBRARY(libEM_CALO.so)
int EM_CALO_MACRO(
    const int nEvents = 1000,
    const string &inputFile =  "/home/keaghan/analysis/sourcefiles/DST_General_particleGun_singleElectron_000_4997000_01000.root",
    ...
    const string &outdir = ".")
{
  ...
  if (Input::SIMPLE)
  {
    INPUTGENERATOR::SimpleEventGenerator[0]->add_particles("e-", 25);
    ...
    INPUTGENERATOR::SimpleEventGenerator[0]->set_eta_range(-3.5, 2);
  ...
  */
  ...
  EM_CALO *em_calo = new EM_CALO("em_calo", outputroot + "_em_calo_testfile.root");
  //anaTutorial->setMinJetPt(3.);
  //anaTutorial->Verbosity(0);
  em_calo->analyzeTracks(true);
  //anaTutorial->analyzeClusters(true);
  //anaTutorial->analyzeJets(true);
  em_calo->analyzeTruth(true);
  em_calo->analyzeTower(true); // process_towers function
